...
- **E2E (`e2e/`, WebdriverIO + real Obsidian 1.13.4):** only seams the mock cannot reach (real `processFrontMatter` serialization, on-disk number-vs-string, self-trigger suppression on real `mtime`, the five bulk modals via DOM, the native settings exclude-list UI). Characterization style: a fresh spec passes on first run. Manual / pre-release - not in CI. First run downloads Obsidian to `e2e/.obsidian-cache/` (gitignored). Full scenario list: `e2e/README.md`.
