...
- Do **not** use an API newer than the pinned `obsidian` types (`~1.13.1`) without first verifying it exists in BOTH the published npm types and the current public Obsidian release - an early-access-only API breaks every public install. Keep the tilde pin. Target is `minAppVersion` 1.13.0 (raised from 1.11.0 for the declarative `getSettingDefinitions()` settings API).
