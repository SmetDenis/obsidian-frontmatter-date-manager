...
**Destructive operation UX**:
Every bulk operation that mutates note data must present a **mandatory dry-run preview** (wizard flow: configure → preview → execute) before any write — the user cannot reach Run without first seeing the exact diff. This preview is the primary safeguard. When the chosen mode is genuinely destructive (overwrite-all populate, fix inversions, rename with delete/overwrite), the Run button must be red via `.setWarning()` and the UI must restate the consequence at the point of action ("cannot be undone" / "irreversible"). Non-destructive modes (fill-missing populate, rebuild cache) keep `.setCta()`. For `BaseBulkModal` subclasses, switch the button via the `isRunDestructive()` hook; the wizard modals (no `BaseBulkModal`) decide per-phase from their selected mode. Bulk buttons are placed last in the settings section. Use `.setWarning()` for the red style — `ButtonComponent.setDestructive()` is 1.13-only and forbidden (see Target note).
