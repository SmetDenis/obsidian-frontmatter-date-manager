...
- **`src/BaseBulkModal.ts`** — Abstract base class for bulk file operations. Provides progress bar, error counting, Run/Cancel UI. Four optional hooks for richer modals: `narrowFiles(files)` (filter the scanned set after `getAllFilesPossiblyAffected`), `renderExtraSection(parent, files)` (inject custom UI between warning and Run/Cancel), `canRun(files)` (gate the Run button — default: non-empty list), `isRunDestructive()` (default `false`; override to `true` to render Run red via `setWarning()` instead of `setCta()`). Extended by `UpdateAllCacheData` (non-destructive) and `FindInversionsModal` (destructive).
...
Never pass `{ ctime, mtime }` to `processFrontMatter()` to preserve timestamps — it prevents Obsidian's editor from reflecting changes (the editor doesn't re-render if mtime is unchanged). Instead, call `processFrontMatter()` without the options argument and use `lastPluginWriteMtime` Map to detect and skip self-triggered modify events: store `file.stat.mtime` immediately after the write, then check it in `handleFileChange` before processing. This pattern applies to ALL automated `processFrontMatter` calls (`handleFileChange`, `handleFileOpen`) **and to every bulk modal write** — `FindInversionsModal.processFile`, `BulkPopulateTimestampsModal.applyTimestamps`, `RenameKeyModal.applyRename`, and `ReformatDateModal.applyReformat` all omit `{ ctime, mtime }`, then set `lastPluginWriteMtime` and refresh the hash cache via `populateCacheForFile`. Preserving mtime is forbidden even for bulk operations: it stops an open editor from re-rendering the change and leaves the hash cache stale, which lets a self-triggered modify event that escapes the `bulkRunning` window spuriously re-stamp `updated`. `bulkRunning` alone is insufficient because those events arrive asynchronously (after the modify debounce), often once `bulkRunning` has already reset to `false`.
...
Bulk operations (populate from filesystem, rebuild cache, find inversions) skip the content hash check in `shouldFileBeIgnored()` via the `BaseBulkModal.skipHashCheck()` hook (default `false`, overridden to `true` where needed). The hash check exists only for automatic change detection on `modify` events — it must not filter files during intentional bulk processing. The flag is applied at the file-listing stage via `getAllFilesPossiblyAffected({ skipHashCheck })`. Per-file, the bulk modals write frontmatter directly (never through `handleFileChange`), set `lastPluginWriteMtime`, and refresh the hash cache.

**Destructive operation UX**:
Every bulk operation that mutates note data must present a **mandatory dry-run preview** (wizard flow: configure → preview → execute) before any write — the user cannot reach Run without first seeing the exact diff. This preview is the primary safeguard. The unifying invariant: red iff the operation replaces or deletes existing user values; blue (`.setCta()`) iff it only adds missing values or touches sidecar data. Destructive cases — overwrite-all populate, fix inversions, rename with delete/overwrite, and reformat (which always rewrites existing date strings in place, so its Run is unconditionally red) — must use red `.setWarning()` and restate the consequence at the point of action ("cannot be undone" / "irreversible"). Non-destructive modes (fill-missing populate, pure-copy rename, rebuild cache) keep `.setCta()`. For `BaseBulkModal` subclasses, switch the button via the `isRunDestructive()` hook; the wizard modals (no `BaseBulkModal`) decide per-phase from their selected mode. Bulk buttons are placed last in the settings section. Use `.setWarning()` for the red style — `ButtonComponent.setDestructive()` is 1.13-only and forbidden (see Target note).
