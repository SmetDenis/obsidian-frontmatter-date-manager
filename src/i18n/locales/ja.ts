  ...
  settings: {
    ...
    advanced: {
      ...
      skipRenameLinkUpdates: {
        ...
        desc: '試験的な機能です。ノートの名前を変更すると、そのノートにリンクしているノートは新しい日付ではなく既存の日付をそのまま保ちます。[[wikilink]] 形式のリンクと、フォルダーではなく単一のノートの名前変更にのみ対応します。まれに、リンクだけを変更した自分の編集で日付が更新されないことがあります。',
      },
      postUpdateCommand: {
        name: '更新後のコマンド',
