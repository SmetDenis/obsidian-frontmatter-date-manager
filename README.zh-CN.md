...
| 实验性：重命名笔记后不更新日期 | `false` | 链接到被重命名笔记的笔记会保留原有日期。仅支持 `[[wikilink]]` 样式链接和单个笔记重命名 |
...
可以部分关闭，默认是关闭的。重命名笔记时，Obsidian 会改写其他笔记里指向它的链接——那些笔记在磁盘上确实发生了变化，所以即使你没有打开过也会获得新日期。开启 **设置 -> 高级 -> 实验性：重命名笔记后不更新日期**，这些笔记就会保留原有日期。

该功能刻意保守：被错误保留的日期会悄无声息地丢失，而多余的日期只是观感问题。插件会预测 Obsidian 即将写入的确切文本，只有文件逐字节匹配时才保留日期。因此它**不适用于**：`[文本](note.md)` 形式的 Markdown 链接、属性内的链接、重命名或移动整个文件夹、在打开的编辑器中有未保存改动的笔记，以及涉及超过 50 篇笔记的重命名。在这些情况下日期照旧更新。如果你在 Obsidian 的“Update links”对话框打开时自己编辑了笔记，该编辑一定会被记录。
